...
from warpdemux.read_until.read_cache import ReadCache
...
class ReadUntilClient(object):
    ...
    def __init__(
        ...
        if self.CacheType.__name__ == "PreallocAccumulatingCache":
            ...
            self.last_channel = (
                self.connection.device.get_flow_cell_info().channel_count
            )
    ...
    def reset(self):
        ...
        if self.CacheType.__name__ == "PreallocAccumulatingCache":
            self.data_queue = self.CacheType(
                self.first_channel, self.last_channel, self.max_raw_signal
            )
        else:
            self.data_queue = self.CacheType(
                max_raw_signal=self.max_raw_signal, size=self.cache_size
            )
        ...
        if not self.one_chunk:
            if self.filter_strands:
                data = [
                    ...
                    and sum(
                        [
                            classif in self.strand_classes
                            for classif in read.chunk_classifications
                        ]
                    )
                    > len(read.chunk_classifications) / 2
